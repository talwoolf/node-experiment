 ...
 * take radius amount, add gap and then half radius of child
 */

 window.utils.getBoundingBox = function( node )
 {
 	if( node.open === false ) 
 	{ 
 		return null;
 		
 	} // if not open, then return the radius of the node
    var r   = node.radius;
    var cr  = 0;
    ...
    while( --i > -1 )
    {
        ...
        cr = Math.max( cr, child.radius );
    }
    var totalRadius = ( r + ( cr * 2 ) );
    return ( totalRadius + ( node.expandBy * 2 ) );
 }
